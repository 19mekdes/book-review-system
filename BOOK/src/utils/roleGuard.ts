...
export const hasPermission = (
  ...
  if (
    rolePermissions.includes("access_all") ||
  ...
  if (options?.ownerId && options?.currentUserId) {
    if (options.ownerId === options.currentUserId) {
      ...
      if (
        permission.includes("edit_own") ||
        permission.includes("delete_own")
      ) {
        return true;
      }
...
export const canAccessRoute = (
  ...
  if (!hasAccess) {
    return {
      ...
      message:
        options?.message || "You do not have permission to access this page.",
    };
  }
