...
router.get('/stats', async (req, res) => {
    try {
        const totalResult = await database_1.default.query('SELECT COUNT(*) FROM users');
    ...
    `);
        ...
        let newUsers = 0;
        try {
            const monthlyResult = await database_1.default.query(`
...
router.get('/me', auth_middleware_1.authenticate, async (req, res) => {
    try {
       ...
       WHERE u.id = $1`, [userId]);
        ...
        // Get role name
        const roleResult = await database_1.default.query(`SELECT name FROM roles WHERE id = $1`, [user.roleId]);
...
router.get('/me/activity', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const reviews = await database_1.default.query(`SELECT 
         r.id,
         r.book_id,
         b.title,
         r.rating,
 ...
 */
router.get('/me/reading-list', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
...
router.put('/me', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        if (email) {
            const existingUser = await database_1.default.query('SELECT id FROM users WHERE email = $1 AND id != $2', [email, userId]);
...
router.post('/me/password', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        await database_1.default.query('UPDATE users SET password = $1, updated_at = NOW() WHERE id = $2', [hashedPassword, userId]);
        res.json({
            success: true,
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`SELECT id, name, email, "roleId", created_at 
...
router.get('/', auth_middleware_1.authenticate, admin_middleware_1.isAdmin, async (req, res) => {
    try {
        const result = await database_1.default.query(`SELECT u.id, u.name, u.email, u."roleId", r.name as role, u.created_at
       FROM users u
...
router.put('/:id', auth_middleware_1.authenticate, admin_middleware_1.isAdmin, async (req, res) => {
    try {
        ...
        if (email) {
            const existingUser = await database_1.default.query('SELECT id FROM users WHERE email = $1 AND id != $2', [email, id]);
            if (existingUser.rows.length > 0) {
                return res.status(400).json({ error: 'Email already in use' });
            }
        }
...
router.delete('/:id', auth_middleware_1.authenticate, admin_middleware_1.isAdmin, async (req, res) => {
    try {
        ...
        await database_1.default.query('DELETE FROM reviews WHERE user_id = $1', [id]);
