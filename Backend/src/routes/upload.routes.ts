...
router.post('/books/:bookId/cover', authenticate, isAdmin, upload.single('cover'), async (req: any, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE books SET cover_image = $1 WHERE id = $2 RETURNING id, title, cover_image',
      [imageUrl, bookId]
    );
...
router.delete('/books/:bookId/cover', authenticate, isAdmin, async (req, res) => {
  try {
    const bookId = parseInt(req.params.bookId);
    
    // Get current cover image path
    const result = await pool.query('SELECT cover_image FROM books WHERE id = $1', [bookId]);
    ...
    if (coverImage) {
      ...
      await pool.query('UPDATE books SET cover_image = NULL WHERE id = $1', [bookId]);
    }
