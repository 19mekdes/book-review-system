...
export class BookModel {
   ...
   * Create a new book
   */
  static async create(bookData: CreateBookInput): Promise<Book> {
    try {
   ...
   * Update a book
   */
  static async update(id: number, updates: UpdateBookInput): Promise<Book | null> {
    try {
   ...
   */
  static async findByCategory(categoryId: number, limit?: number, offset?: number): Promise<BookWithDetails[]> {
    try {
      let query = `
        SELECT 
          b.id,
   ...
   * Search books
   */
  static async search(searchTerm: string, limit?: number, offset?: number): Promise<BookWithDetails[]> {
    try {
      let query = `
        SELECT 
          b.id,
